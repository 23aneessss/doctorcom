...
interface SidebarProps extends Omit<ComponentPropsWithoutRef<"aside">, "children"> {
	className?: string;
	currentUser?: {
		name: string;
		email: string;
...
const SIDEBAR_ICON_ASSET_MODULES = {
	...import.meta.glob<{ default: string }>("../assets/icons/sidebar/*.svg", { eager: true }),
	...import.meta.glob<{ default: string }>("../assets/icons/sidebar/*.png", { eager: true }),
};

const SIDEBAR_ICON_URLS = Object.entries(SIDEBAR_ICON_ASSET_MODULES).reduce<Record<string, string>>(
	(acc, [path, mod]) => {
...
const PRIMARY_NAV_ITEMS: SidebarRouteItem[] = [
	{ href: "/", label: SIDEBAR_TEXT.accueil, icon: <HomeGlyph />, assetName: "home" },
	{ href: "/patients", label: SIDEBAR_TEXT.patients, icon: <PatientsGlyph />, assetName: "patients" },
	{ href: "/agenda", label: SIDEBAR_TEXT.agenda, icon: <AgendaGlyph />, assetName: "agenda" },
	{
		href: "/ordonnances",
...
const SIDEBAR_STYLE_MAP = {
	...
	logoWrap: {
		...
		height: 54,
		margin: "clamp(10px, 2.2vh, 20px) auto 0",
	} satisfies CSSProperties,
	logoImage: { width: "100%", height: "100%", display: "block", objectFit: "contain" } satisfies CSSProperties,
	navBase: {
		marginLeft: "auto",
		marginRight: "auto",
	} satisfies CSSProperties,
	primaryNav: {
		...
		marginTop: "clamp(12px, 2.8vh, 22px)",
	} satisfies CSSProperties,
	secondaryNav: {
	...
	iconGlyph: { width: 18, height: 18, display: "block" } satisfies CSSProperties,
	iconImage: { width: "100%", height: "100%", display: "block", objectFit: "contain" } satisfies CSSProperties,
	navLabel: { margin: 0, lineHeight: "22px", whiteSpace: "nowrap" } satisfies CSSProperties,
	divider: {
		alignSelf: "stretch",
		height: 0,
	...
	userCard: {
		...
		borderRadius: 11,
		background: "#0f3460",
		boxShadow: "0px 2px 12px 0px rgba(194, 224, 239, 0.22)",
	} satisfies CSSProperties,
	userCardWrap: {
		marginTop: "auto",
		width: "min(190px, calc(100% - 24px))",
	...
	userRow: { display: "flex", alignItems: "center", gap: 7 } satisfies CSSProperties,
	avatar: {
		width: 30,
		height: 30,
		borderRadius: 9999,
	...
	userText: { minWidth: 0 } satisfies CSSProperties,
	userName: {
		margin: 0,
		color: "#fff",
		fontSize: 12,
...
function SidebarIcon({
	...
	return (
		<span aria-hidden="true" style={mergeStyles(SIDEBAR_STYLE_MAP.iconSlot, { opacity })}>
			{useAsset ? (
				<img
					src={iconAssetUrl}
					alt=""
...
function SidebarNavItem({
	...
	const labelStyle = mergeStyles(
		...
		visualState === "active"
			? { color: SIDEBAR_COLORS.textActive, fontSize: 13, fontWeight: 500 }
			: visualState === "hover"
				? { color: SIDEBAR_COLORS.textActive, fontSize: 13, fontWeight: 600 }
				: { color: SIDEBAR_COLORS.textDefault, fontSize: 13, fontWeight: 600 }
	);
...
function SidebarLogo() {
	return (
		<div style={SIDEBAR_STYLE_MAP.logoWrap} aria-label="doctor.com" data-node-id="1:1015">
			<img src={doctorLogo} alt="doctor.com" style={SIDEBAR_STYLE_MAP.logoImage} />
		</div>
	);
}

function SidebarUserCard({ currentUser }: { currentUser: NonNullable<SidebarProps["currentUser"]> }) {
	const avatar = currentUser.avatarUrl ? (
		<img style={SIDEBAR_STYLE_MAP.avatar} src={currentUser.avatarUrl} alt={currentUser.name} />
	) : (
		<span style={SIDEBAR_STYLE_MAP.avatar} aria-hidden="true" />
	);

	return (
		<div style={SIDEBAR_STYLE_MAP.userCard} aria-label="Current user profile" data-node-id="1:1050">
...
function Sidebar({ className, currentUser = DEFAULT_USER, ...props }: SidebarProps) {
	...
	return (
		...
		>
			...
			<nav
				style={mergeStyles(SIDEBAR_STYLE_MAP.navBase, SIDEBAR_STYLE_MAP.primaryNav)}
			...
			<nav
				style={mergeStyles(SIDEBAR_STYLE_MAP.navBase, SIDEBAR_STYLE_MAP.secondaryNav)}

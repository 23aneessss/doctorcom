...
import { CircleHelp, FileStack, FileText, Loader2, Pencil, Plus, Printer, Search, Sparkles, X } from "lucide-react";
...
export function NouvelleOrdonnanceDialog({
  ...
  const [createdOrdonnanceId, setCreatedOrdonnanceId] = useState<string | null>(null);
  const [activeAiPanel, setActiveAiPanel] = useState<"ordonnance" | "medicament" | null>(null);
  ...
  const preRemplisByCategorieQuery = useQuery({
    ...trpc.ordonnance.getPreRemplisByCategorie.queryOptions({
      categorieId: selectedCategorieId || "00000000-0000-0000-0000-000000000000",
    }),
  ...
  const filteredPreRemplis = useMemo(() => {
    return preRempliItems.filter((item) => {
      const bySpecialite =
        !selectedSpecialite ||
        (item.specialite ?? "").toLowerCase() === selectedSpecialite.toLowerCase();
  ...
  const templateItemsByExternalId = useMemo(() => {
    ...
    return new Map(templateItems.map((item) => [item.medicament_externe_id, item]));
  ...
  const preRempliHasExtras = useMemo(() => {
    ...
    return rows.some(
      (row) =>
        Boolean(row.medicament_externe_id) && !templateItemsByExternalId.has(row.medicament_externe_id),
    );
  }, [mode, rows, templateItemsByExternalId]);

  const rendezVousTerminesForSuivi = useMemo(() => {
    return rendezVous
      .filter((item) => item.suivi_id === selectedSuiviId && item.statut === "termine")
      .sort((left, right) => {
  ...
  useEffect(() => {
    ...
    const stillExists = templateList.some((item) => item.id === selectedPreRempliId);
  ...
  useEffect(() => {
    ...
    if (isRowsDirty && rows.some((row) => row.nom_medicament || row.posologie || row.dosage)) {
  ...
  }, [open, mode, preRempliDetailQuery.data, appliedTemplateId, isRowsDirty, rows]);
  ...
  const createOrdonnanceMutation = useMutation({
    mutationFn: async () => {
      ...
      if (mode === "pre-remplie" && !selectedPreRempliId) {
        throw new Error("Choisissez un modèle pré-rempli avant l'enregistrement.");
      }
      ...
      if (mode === "pre-remplie" && selectedPreRempliId) {
        ...
        const hasExtras = rows.some(
          (row) =>
            row.medicament_externe_id && !templateExternalIds.has(row.medicament_externe_id),
        );
        ...
        if (!hasExtras) {
          ...
          for (const templateItem of templateItems) {
            const row = rowsByExternalId.get(templateItem.medicament_externe_id);
            if (!row) {
              modifications.push({
            ...
            const posologieChanged = posologie !== (templateItem.posologie_defaut ?? "").trim();
            const dureeChanged = duree !== (templateItem.duree_defaut ?? null);
            const instructionsChanged =
              instructions !== (templateItem.instructions_defaut ?? null);

            if (dosageChanged || posologieChanged || dureeChanged || instructionsChanged) {
              modifications.push({
                medicament_externe_id: templateItem.medicament_externe_id,
                dosage,
                posologie,
          ...
          const result = await trpcClient.ordonnance.creerOrdonnanceDepuisPreRempli.mutate({
      ...
      const result = await trpcClient.ordonnance.creerOrdonnance.mutate({
        ...
        pre_rempli_origine_id: mode === "pre-remplie" ? selectedPreRempliId || null : null,
        medicaments,
      });
  ...
  const previewMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ordonnanceId) {
        throw new Error("Enregistrez d'abord l'ordonnance pour la prévisualiser.");
      }
  ...
  }, [open, rows, remarques, selectedSuiviId, selectedRendezVousId, mode, selectedPreRempliId]);
  ...
  }, [open, onOpenChange, activeAiPanel]);
  ...
  const isBusy = createOrdonnanceMutation.isPending || previewMutation.isPending;
  const showManualEditor = mode === "manuel";
  const hasRightPanel = activeAiPanel !== null;
  ...
  const applyTemplateRows = async (
    ...
    try {
      const detail = await trpcClient.ordonnance.getPreRempliById.query({ id: preRempliId });
      ...
      toast.error(error instanceof Error ? error.message : "Impossible de charger le modèle.");
    } finally {
      setTemplateActionLoadingId(null);
    }
  };
  ...
  return (
    <>
      ...
      >
        <div
          className={`my-4 flex w-full flex-col items-stretch gap-3 ${
            hasRightPanel
              ? "max-w-[1240px] lg:flex-row lg:items-start"
              : "max-w-[600px] items-center"
          }`}
        >
          <div className="flex w-full max-w-[600px] flex-col overflow-hidden rounded-[14px] bg-white shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)]">
          ...
          <div className="consultation-modal-scrollbar flex max-h-[82vh] flex-col overflow-y-auto px-5 pb-4 pt-5">
            <div className="mb-4 grid grid-cols-2 gap-3">
              <button
                className={`inline-flex h-[37px] cursor-pointer items-center justify-center gap-2 rounded-[10px] px-4 font-['Plus_Jakarta_Sans'] text-[14px] font-medium transition-colors ${
                  activeAiPanel === "ordonnance"
                    ? "bg-[#265284] text-white hover:bg-[#1f436d]"
                    : "bg-[#76bbdd] text-white hover:bg-[#63b0d6]"
                }`}
                onClick={() => {
                  setActiveAiPanel((current) => (current === "ordonnance" ? null : "ordonnance"));
                }}
                type="button"
              >
                <Sparkles className="size-4" />
                <span className="whitespace-nowrap">Recommandation ordonnance</span>
              </button>

              <button
                className={`inline-flex h-[37px] cursor-pointer items-center justify-center gap-2 rounded-[10px] px-4 font-['Plus_Jakarta_Sans'] text-[14px] font-medium transition-colors ${
                  activeAiPanel === "medicament"
                    ? "bg-[#265284] text-white hover:bg-[#1f436d]"
                    : "bg-[#76bbdd] text-white hover:bg-[#63b0d6]"
                }`}
                onClick={() => {
                  setActiveAiPanel((current) => (current === "medicament" ? null : "medicament"));
                }}
                type="button"
              >
                <Sparkles className="size-4" />
                <span className="whitespace-nowrap">Recommandation medicaments</span>
              </button>
            </div>
                ...
                <div className="consultation-modal-scrollbar mt-3 flex max-h-[365px] w-full flex-col gap-3 overflow-y-auto pr-1">
                  {filteredPreRemplis.length === 0 ? (
                    ...
                    filteredPreRemplis.map((preRempli) => {
                      const medicamentsCount = preRempliCountById.get(preRempli.id) ?? 0;
                      const isLoading = templateActionLoadingId === preRempli.id;
                          ...
                          <div className="grid grid-cols-[1fr_auto] gap-3">
                            <div className="space-y-2">
                              ...
                              <p className="font-['Inter'] text-[12px] text-[#415c7b]">
                                {preRempli.description || "Modèle pré-rempli de prescription médicale"}
                              </p>
                            ...
                            <div className="flex flex-col gap-[10px] px-[12px] pb-[14px]">
                              <button
                                className={`h-[29px] w-[72px] cursor-pointer rounded-[12px] font-['Plus_Jakarta_Sans'] text-[14px] font-medium text-white shadow-[0px_4px_12px_0px_rgba(118,187,221,0.5)] transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                                  isSelected ? "bg-[#265284] hover:bg-[#1f436d]" : "bg-[#76bbdd] hover:bg-[#63b0d6]"
                                }`}
                                disabled={isLoading}
                                onClick={() => void applyTemplateRows(preRempli.id, "pre-remplie")}
                                type="button"
                              >
                              ...
                              <button
                                ...
                                disabled={isLoading}
                                onClick={() => void applyTemplateRows(preRempli.id, "manuel")}
                                type="button"
                              >
                                Modifier
                              </button>
            ...
            {showManualEditor ? (
              <div className="mt-4 space-y-3">
                {rows.map((row, index) => (
                  <div key={row.id} className="rounded-[10px] border border-[#c2e0ef] bg-[#f8fafc]">
                    ...
                    <div className="space-y-2 p-3">
                      <div className="relative">
                        <input
                              ...
                              prev.map((item) =>
                                item.id === row.id
                                  ? {
                                      ...
                                      medicament_externe_id:
                                        value.trim() !== item.nom_medicament.trim()
                                          ? ""
                          ...
                          onBlur={() => {
                            window.setTimeout(() => {
                              setActiveSearchRowId((current) => (current === row.id ? null : current));
                            }, 120);
                          }}
                          onFocus={() => {
                                        ...
                                        current.id === row.id
                                          ? {
                                              ...
                                              posologie:
                                                ...
                                                "",
                                              dosage: current.dosage || item.dose_maximale || "",
                                              instructions:
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          onChange={(event) => {
                            const value = event.target.value;
                            setRows((prev) =>
                              prev.map((item) => (item.id === row.id ? { ...item, posologie: value } : item)),
                            );
                            setIsRowsDirty(true);
                          }}
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            setRows((prev) =>
                              prev.map((item) =>
                                item.id === row.id ? { ...item, duree_traitement: value } : item,
                              ),
                            );
                            setIsRowsDirty(true);
                          }}
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          const value = event.target.value;
                          setRows((prev) =>
                            prev.map((item) => (item.id === row.id ? { ...item, dosage: value } : item)),
                          );
                          setIsRowsDirty(true);
                        }}
                        placeholder="Dosage"
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          setRows((prev) =>
                            prev.map((item) =>
                              item.id === row.id ? { ...item, instructions: value } : item,
                            ),
                          );
                          setIsRowsDirty(true);
                        }}
            ...
            {!selectedRendezVousId && selectedSuivi ? (
              <p className="mt-3 rounded-[10px] border border-[#f97316] bg-[#fff7ed] px-3 py-2 font-['Inter'] text-[12px] text-[#b45309]">
                Aucun rendez-vous terminé trouvé pour ce suivi. La création d'ordonnance est bloquée.
              </p>
            ) : null}
          ...
          {activeAiPanel === "ordonnance" ? (
            <div className="w-full max-w-[600px] self-center transition-[opacity,transform] duration-200 ease-out lg:self-start">
              <OrdonnanceSugAiDialog
                onAccept={(recommendation: OrdonnanceAiRecommendation) => {
                  ...
                  setIsRowsDirty(true);
                  setRows(
                    recommendation.ordonnance_draft.medicaments.map((item) => ({
                      id: crypto.randomUUID(),
                      medicament_externe_id: item.medicament_externe_id,
                      nom_medicament: item.nom_medicament,
                      dosage: item.dosage ?? "",
                  ...
                  setRemarques(recommendation.ordonnance_draft.remarques ?? "");
                }}
          ...
          {activeAiPanel === "medicament" ? (
            <div className="w-full max-w-[613px] self-center transition-[opacity,transform] duration-200 ease-out lg:self-start">
              <MedicamentSugAiDialog
                ...
                onSelectMedicament={(item) => {
                  const existingIndex = rows.findIndex(
                    (row) => row.medicament_externe_id === item.medicament_externe_id,
                  );

                  if (existingIndex >= 0) {
                    toast.info(`${item.nom_medicament} est déjà présent dans l'ordonnance.`);
                    return;
                  }

  ...
  return (
    <div className="flex h-[60.8px] shrink-0 items-center justify-between border-b-[0.8px] border-[#c2e0ef] bg-[#f8fafc] px-5">
      <div className="flex items-center gap-3">
        <Syringe className="size-5 text-[#0f3460]" />
    ...
    <form.Field name={name}>
      {(field: any) => (
        <div className="space-y-[6px]">
          ...
          <textarea
            className="h-[77.6px] w-full resize-none rounded-[10px] border-[0.8px] border-[#c2e0ef] bg-white px-3 py-2 font-['Inter'] text-[14px] leading-5 text-[#0f3460] outline-none transition-colors placeholder:text-[rgba(100,116,139,0.9)] focus:border-[#76bbdd] focus:ring-2 focus:ring-[#c2e0ef]/50"
            onBlur={field.handleBlur}
            onChange={(e) => field.handleChange(e.target.value)}
            placeholder={placeholder}
            value={field.state.value}

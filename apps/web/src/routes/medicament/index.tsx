...
import { ModifierMedicamentDialog } from "./popups/-modifier-medicament";
...
function RouteComponent() {
  ...
  const [editMedicamentId, setEditMedicamentId] = useState<number | null>(null);
  ...
  return (
    <div className="flex h-svh bg-[#FBFBFC]">
        ...
        <div className="mx-auto w-[1220px] max-w-[calc(100%-48px)] pb-[61px] pt-[41px]">
          ...
          <section className="mt-[33px] w-full">
            ...
            {mobileCatalogQuery.isLoading || advancedSearchQuery.isLoading ? (
              ...
              <div className="ml-[67px] mt-[30px] w-[1086px] max-w-[calc(100%-67px)] rounded-[16px] border border-[#c2e0ef] bg-white p-8 font-['Inter'] text-[14px] text-[#0f3460] shadow-[0px_4px_20px_rgba(194,224,239,0.2)]">
              ...
              <div className="ml-[67px] mt-[30px] grid w-[1086px] max-w-[calc(100%-67px)] grid-cols-1 gap-x-[41px] gap-y-[44px] xl:grid-cols-3">
                {displayedItems.map((medication) => (
                  <MedicationCard
                    ...
                    onEdit={setEditMedicamentId}
                    onView={setViewMedicamentId}
      ...
      <VoirMedicamentDialog
        ...
        open={viewMedicamentId !== null}
      />

      <ModifierMedicamentDialog
        medicamentId={editMedicamentId}
        onOpenChange={(open) => {
          if (!open) {
            setEditMedicamentId(null);
          }
        }}
        onUpdated={() => {
          void refreshQueries();
        }}
        open={editMedicamentId !== null}
      />
    </div>
  );
}

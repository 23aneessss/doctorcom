...
const createExamenInputSchema = z.object({
  ...
  tension_arterielle: z.string().trim().max(32).optional().nullable(),
  frequence_cardiaque: z.number().int().min(0).max(300).optional().nullable(),
  temperature: z.number().min(30).max(45).optional().nullable(),
  spo2: z.number().min(0).max(100).optional().nullable(),
  imc: z.number().min(0).optional().nullable(),
